<template>
  <div>
    <div
      v-if="editor"
      class="editor-container bg-white rounded border border-amber-50 shadow-md"
    >
      <div
        class="toolbar grid grid-cols-5 md:grid-cols-6 gap-1 bg-purple-500 p-2 rounded-t"
      >
        <button
          class="btn-editor"
          type="button"
        ...
        <button
          ...
          type="button"
          @click="toggleStrike"
          :class="{ 'btn-editor-active': editor.isActive('strike') }"
        >
        ...
        <button
          ...
          :class="{ 'btn-editor-active': editor.isActive('blockquote') }"
        >
...
onMounted(() => {
  editor.value = new Editor({
    ...
    editorProps: {
      attributes: {
        class:
          'border-gray-400 p-4 min-h-[25rem] rounded-b-xl max-h-[25rem] overflow-y-auto p-4 outline-none prose',
      },
    },
  })
})
